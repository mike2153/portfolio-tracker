/*
 * Centralised thin client for calling the backend FastAPI routes from the Expo app.
 * This is a cross-platform version of the Next.js front_api_client.
 * Uses the same API endpoints but with cross-platform auth handling.
 */

import { supabase } from './supabase';
import { ApiResponse, DashboardOverview } from '../models/api';
...
async function authFetch(path: string, init: RequestInit = {}) {
  ...
  if (!session?.access_token) {
    ...
    if (refreshError) {
      console.error('[authFetch] Refresh error:', refreshError);
  ...
  try {
    ...
    console.error('[authFetch] Fetch error:', fetchError);
    throw fetchError;
  }
}
...
async function getJSON<T>(path:string): Promise<T> {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[getJSON] Error:', errorMessage, errorBody);
    ...
    try {
      ...
      console.error('[getJSON] Parse error:', jsonError);
      throw new Error(`Failed to parse JSON response from ${path}`);
    }
  } catch (error) {
    console.error('[getJSON] Request error:', error);
    throw error;
  }
}
...
export const front_api_get_dashboard = (): Promise<ApiResponse<DashboardOverview>> => {
  ...
  // Add promise logging
  result.then((data) => {
    console.log('[front_api_get_dashboard] Success:', data);
  }).catch((error) => {
    console.error('[front_api_get_dashboard] Error:', error);
  });
...
// Export apiClient for compatibility with components that use this pattern
export const apiClient = {
  get,
};

// Export authFetch for direct use
export { authFetch };